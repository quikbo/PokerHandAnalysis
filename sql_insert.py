...
def parse_text_file(filename: str) -> Dict:
    """Read and parse the poker hand text file into a structured dictionary"""
    data = {}
...
class SQLGenerator:
    ...
    def determine_action_type(self, action: str, street: str, previous_actions: List[str]) -> str:
        """Determine specific action type based on context"""
        parts = action.split()
        action_code = parts[1]
        ...
        if action_code == 'f':
            ...
            # Determine if it's a check or call by looking at previous actions in the street
            is_check = True
            current_street_actions = []
            for prev_action in previous_actions:
                prev_parts = prev_action.split()
                if prev_parts[0] == 'd' and prev_parts[1] == 'db':  # New street marker
                    current_street_actions = []
                elif prev_parts[0] != 'd':  # Regular action
            ...
            # Determine if it's a bet or raise by looking at previous actions
            is_bet = True
            current_street_actions = []
            for prev_action in previous_actions:
                prev_parts = prev_action.split()
    ...
    def generate_sql_statements(self, hand_data: Dict, file_name: str) -> List[str]:
        sql_statements = []
        ...
        for i, action in enumerate(hand_data['actions']):
            parts = action.split()
            if parts[0] not in ['d']:  # Skip deal actions
...
def process_directory(directory_path: str, output_file: str = "poker_hands.sql"):
    ...
    with open(output_file, 'w') as out_file:
        ...
        # Get folders and sort numerically
        folders = []
        ...
        for _, folder_name in folders:
            ...
            try:
                ...
                # Process each .phh file in the subfolder with numerical sorting
                phh_files = []
                for f in os.listdir(folder_path):
                    if f.endswith('.phh'):
                        try:
                            # Extract number from filename (removing '.phh')
                            file_num = int(f[:-4])
                ...
                for _, file_name in phh_files:
                    ...
                    try:
                        ...
                        sql_statements = sql_generator.generate_sql_statements(hand_data, file_name)
